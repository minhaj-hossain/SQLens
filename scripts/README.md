...
| `audit-ddl-contracts.ts` | `npm run audit:ddl-contracts` | Workstream A gate (DDL): every column a CREATE/ALTER `solutionSql` declares must be (1) visible in the rendered prompt and (2) declared in `validation.requiredColumns`; phantom required columns are flagged too. Days 27-30 block, other days advisory. Exit 1 on blocking findings. |
