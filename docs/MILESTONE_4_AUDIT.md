...
## P2 Execution Log (2026-09-23) — all items done, all gates green

### 9 + 11. Visualizer variants wired, suppression synced
- **CtePipelineVisualizer** (recursive + hierarchy laps), **TransactionTimelineVisualizer**
  (interleaved isolation lanes / atomicity / deadlock wait-for ring),
  **BTreeIndexVisualizer** (composite multi-key compare / covering index-only seek),
  **SchemaBlueprintVisualizer** (blueprint / expand→backfill→contract migration).
- `ConceptMentalModel.tsx` module defaults: day-41/42 → `SUBQUERY_CTE`,
  day-49/50 → `DML_TRANSACTION`, day-52 → `BTREE_INDEXING`, day-56 → `SCHEMA_DDL`;
  per-concept views stay in `CONCEPT_VARIANT_OVERRIDES`. Variant resolution is
  archetype-aware (a single-view archetype never receives an orphan variant
  string — pinned by the gate's `invalid-variant` rule).
- One rejected detour, documented in the file: a parallel
  `M4_CONCEPT_OVERRIDES` map with invented archetype names failed type-checking
  (31 TS errors) and was fully unwound; the shipped design keeps ONE archetype
  namespace.

### 12. New gate `audit:visual-coverage` — wired into `audit:all`
- Four rules: (1) no naked fallback — `TABULAR_PROJECTION` requires an
  `introTable`; (2) suppression sync — diagram days must be listed in
  `shouldSuppressTopIntroTable`, TABULAR concepts never suppressed;
  (3) `stepBreakdowns` require a `targetQuery`; (4) no dead or invalid
  override entries.
- Baseline captured in `docs/P2_GATE_BASELINE.txt`: **27 blocking / 17 advisory**.
- Days 39–57 block; Days 1–38 print advisory only (pre-M4 debt, deliberately
  left advisory).

### 10a. 26 authentic `introTable`s + 1 Day-57 re-route
- Sourcing rule held throughout: every row comes from the concept's own
  `liveDemoSql` executed on the seeded engine, from other real engine output
  (EXPLAIN plans, view results, constraint errors, trigger logs), or from a
  SQL-standard / lesson-own fact matrix. **No invented millisecond timings**
  (day-53's pre-existing `(1ms)` prose was logged, not extended).
- Coverage: day-39 ×2, day-40 ×2, day-43 ×2, day-44 ×2, day-45 ×2, day-46 ×2,
  day-49 ×1, day-51 ×3, day-53 ×3, day-54 ×3, day-55 ×2, day-57 ×2 = **26**.
- Day-57 `capstone-composite-indexing` re-routed to `BTREE_INDEXING` /
  variant `composite` (the existing B-tree visualizer); day-57 added to
  `shouldSuppressTopIntroTable` — its two TABULAR concepts are unaffected
  because TABULAR is never suppressed.

### 10b. `targetQuery` + `stepBreakdowns` for each day's T1 concept
- Days 40–57 concept 1 each gained a hero query + three step breakdowns;
  day-39 concept 1 already had its `targetQuery` and gained the matching
  steps. Every `sqlSnippet` is drawn from that concept's own demo/example
  SQL — 19/19 M4 days now satisfy "target query before steps".

### P2-A. Unknown columns are named errors (engine)
- `executor.ts`: projection / WHERE / function-argument references to unknown
  columns now fail with MySQL wording (`Unknown column 'x' in 'field list'` /
  `'where clause'`); unresolvable relations (CTE bodies) still skip the guard;
  routine bodies keep `suppressIdentCheck`.
- 7 regression tests — `tests/engine/unknown-columns.test.ts`.
- `day48-t1` and `tests/engine/set-ops.test.ts` fixtures corrected: they had
  been passing only through the silent-NULL bug (`total_amount` /
  `suppliers.city` do not exist).

### P2-B. `CALL` binds procedure parameters (engine) — discovered during 10a
- Probe evidence: `CALL sp_update_order_status(2, 'completed')` reported
  `Updated 0 row(s)` — the parameter resolved as NULL (ident-guard suppressed
  for routine bodies), so `order_id = NULL` matched nothing and CALL
  "succeeded" while silently doing nothing. `day44-t2` passed only because
  its validation is structural.
- Fix: arity-checked argument evaluation, quote/qualification-aware
  identifier substitution into the body, multi-statement bodies run in order
  and stop at the first failure, bare unresolvable argument → named error.
- 6 regression tests — `tests/engine/procedure-params.test.ts`.

### Final verification (full CI parity)
`lint` 0 · `npm test` **641/641** (56 files; +13 engine tests) ·
`audit:all` **11/11 green** with `audit:visual-coverage` newly wired
(**0 blocking / 17 advisory**; baseline was 27/17) · `npm run build` ✅.

---

## P3 Execution Log (2026-09-23) - items 13-16 done, all gates green

### 13. API rate limiting (S-1) - zero-dep, bucketed, live-verified
- `src/lib/rate-limit.ts`: fixed-window limiter, **no new npm deps** (repo
  convention), injectable clock so tests never sleep, Map swept past 512
  entries. Key = bucket + client IP + djb2-hashed better-auth session token
  (raw token never stored); IP from `x-forwarded-for` first hop, then
  `x-real-ip`, else `unknown`.
- Budgets sized to the app's real cadence: GET `/api/auth/*` 120/min (session
  reads), POST `/api/auth/*` 30/min (brute-force writes), PUT
  `/api/me/progress` 120/min (client debounces at 1.5s ~ 40/min steady).
- Enforced ahead of auth/Mongo work: wrapped around the better-auth catch-all
  handlers and at the top of the progress PUT. 429 is a friendly JSON body
  (`error: rate_limited`, plain-language `message`, `retryAfterSeconds`) plus
  a `Retry-After` header; the provider's existing non-OK path already retries
  with backoff, so a throttled push degrades gracefully (localStorage holds
  the data).
- **Live probe caught a real bug pre-commit**: GET and POST initially shared
  one window counter (the rule `max` differed, the key did not) - 35
  `get-session` GETs exhausted the POST budget. Fixed with per-endpoint
  buckets (`auth-read` / `auth-write` / `progress-write`) + regression test.
- Live evidence (`next start`): unauthenticated PUT -> 120x401, request #121
  -> our exact 429 body with `retryAfterSeconds: 60`. Auth POST passes
  requests through our layer into better-auth, which has its own stricter
  built-in limiter (`dist/api/rate-limiter`, defaults - its 429 starts at
  attempt 4); our budget sits above it as the uniform ip+session layer and is
  the only limiter on the progress PUT.
- Known limit: the window Map is per server instance (serverless = per
  instance). A WAF rule or Upstash-style shared limiter remains the
  recommended outer layer for a *global* quota - noted, not implemented.
- Tests: `tests/rate-limit/rate-limit.test.ts` - 15 (window expiry/reset by
  injected clock, bucket isolation, key hashing, cookie/IP parsing, pruning,
  shipped-policy sanity).

### 14. Security headers + CSP (S-4)
- `next.config.ts headers()` on `/(.*)`: `X-Content-Type-Options: nosniff`,
  `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`
  always; `Content-Security-Policy` and `Strict-Transport-Security` only when
  `NODE_ENV === 'production'` (the dev overlay/HMR needs eval + ws:, and dev
  is never exposed).
- CSP = conservative `default-src 'self'` with `script-src`/`style-src`
  `'self' 'unsafe-inline'`: the App Router ships its RSC flight payload and
  the layout theme script inline; a nonce-based CSP needs the Next 16 proxy
  layer (recorded in the config comment as follow-up). No external origins
  exist to allow - grep over `layout.tsx` finds only schema.org/SITE_URL
  metadata literals.
- Live evidence: all 5 headers observed on `GET /signin` under `next start`
  (probe printed nosniff, DENY, strict-origin-when-cross-origin, full CSP,
  `max-age=31536000; includeSubDomains`).

### 15. Dev time knobs inert in production (S-3)
- New `src/lib/progress/dev-knobs.ts` - `devKnobsEnabled()` =
  `process.env.NODE_ENV === 'development'`. Applied at every reader:
  `getEffectiveNow` (offset math, which also feeds `formatTimeRemaining` and
  `ModuleCompletionView`), the `bypassDailyLock` unlock branch, and both merge
  paths (`fromCloudProgress`, `mergeProgress`). Types untouched;
  `loadUserState` already hard-resets both fields on every load.
- **S-3 correction:** the finding's "Header UI" claim is stale - repo-wide
  grep found no knob setters anywhere in `src` (README already notes the
  legacy bypass toggles were retired). Knobs were only reachable through
  hand-built/merged state, which is now gated at runtime.
- Tests: `tests/progress/dev-knobs.test.ts` - 6 (inert in test+production,
  active in development, unlock divergence dev vs prod, both merge paths,
  cloud-strip invariant).

### 16. Grading/cloud-progress integrity stance documented
- `docs/GRADING_POLICY.md` new section "Grading integrity vs. cloud progress
  (Milestone 4, S-2)": cloud progress never backs a certificate, badge,
  analytics claim, or leaderboard; any future credential feature must
  re-grade server-side; the admin `audit_log` (S-5, **not yet implemented**)
  would be admin-mutations-only and never a mastery record; dev knobs inert
  outside `next dev`.
- `npm run audit:grading-policy` re-run after the edit: green (424 tasks, 0
  findings).

### S-findings status after P3
- S-1 mitigated in-repo (instance-scoped; WAF/Upstash still recommended for a
  global quota) - S-2 stance documented - S-3 closed (gated + stale claim
  corrected) - S-4 closed (headers + CSP + HSTS) - S-5 stance documented,
  implementation remains open backlog - S-6/S-7 unchanged (accepted).

### Final verification (P3)
`lint` 0 - `npm test` **662/662** (58 files; +21 P3 tests) - `audit:all`
**11/11 green** (**0 blocking / 17 advisory**, unchanged from P2) -
