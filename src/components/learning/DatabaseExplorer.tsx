    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 px-3 sm:px-4 py-2.5 sm:py-3.5 bg-surface border-b border-border-soft min-w-0">
        <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
          <Database className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-text-dim shrink-0" />
          <div className="relative inline-block">
            <select
              ...
              className="appearance-none font-mono text-xs font-bold text-text bg-surface-2 border border-border rounded-md pl-2.5 pr-6 sm:pr-7 py-1 focus:outline-none focus:border-func transition cursor-pointer max-w-[150px] sm:max-w-none truncate"
            >
        ...
        {/* Right: Tabs & Mobile Collapse Toggle */}
        <div className="flex items-center gap-2 sm:gap-4 ml-auto">
          {/* Underline Tabs (design `.tabs` / `.tab`) */}
          <div className={`${isCollapsedOnMobile ? 'hidden sm:flex' : 'flex'} items-center gap-2.5 sm:gap-4 text-[11px] sm:text-[11.5px] font-mono`}>
            <button
            ...
            <button
              ...
              className={`hidden sm:flex items-center gap-1 pb-1 border-b-2 transition cursor-pointer ${
                activeTab === 'graph'
                  ? 'text-text font-semibold border-text'
                  : 'text-text-faint hover:text-text-dim border-transparent'
              }`}
          ...
          <button
            ...
            className="sm:hidden flex items-center gap-1 text-[11px] font-mono text-text-dim hover:text-text px-2 py-1 rounded bg-surface-2 border border-border cursor-pointer"
          >
            <span>{isCollapsedOnMobile ? 'Expand' : 'Collapse'}</span>
            <ChevronDown className={`w-3 h-3 transition-transform ${isCollapsedOnMobile ? '' : 'rotate-180'}`} />
          </button>
        </div>
      </div>

      {/* Main Body Grid — collapsible on mobile */}
