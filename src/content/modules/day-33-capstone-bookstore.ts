      ...
      tasks: [
        {
          ...
          validation: {             requiredColumns: ['publisher_id', 'name', 'author_id'],             targetTable: 'publishers', expectedRowCount: 1,           },
        ...
        },
        {
          ...
          validation: {             requiredColumns: ['book_id', 'title', 'author_id', 'publisher_id', 'genre', 'price', 'quantity_in_stock'],             targetTable: 'books', expectedRowCount: 1,           },
        ...
        },
        {
          ...
          validation: {             requiredColumns: ['sale_id', 'book_id', 'sale_date', 'quantity', 'unit_price'],             targetTable: 'sales', expectedRowCount: 1,           },
      ...
      tasks: [
        {
          ...
          instructions: [
            ...
            "Specify the column type as `VARCHAR(20)` with a default value of `'first'`.",
          ...
          validation: {             requiredColumns: ['edition'],             targetTable: 'books', expectedRowCount: 1,           },
        ...
        },
        {
          ...
          instructions: [
            ...
            "In the same execution, run an EXPLAIN query filtering `books` where `title` equals 'Atomic Habits'.",
            'Verify that the execution plan demonstrates access type `ref` using `idx_books_title`.',
          ],
          type: 'independent',
          primaryTable: 'books',
        ...
        },
        {
          ...
          instructions: [
            ...
            "Insert a test book with title 'Rollback Me', author_id 1, publisher_id 1, genre 'Test', price 9.99, and quantity 5.",
            'Roll back the transaction to undo the insertion.',
            'Run a verification query selecting `COUNT(*) AS cnt` from `books` to confirm the book count remains at 4.',
          ],
          type: 'challenge',
          primaryTable: 'books',
    ...
    tasks: [
      {
        ...
        instructions: [
          ...
          'Create `books (book_id INT PRIMARY KEY AUTO_INCREMENT, title VARCHAR(200) NOT NULL, author_id INT NOT NULL, publisher_id INT NOT NULL, genre VARCHAR(50) NOT NULL, price DECIMAL(8,2) NOT NULL CHECK (price >= 0), quantity_in_stock INT NOT NULL DEFAULT 0)` with both foreign keys (author_id → authors, publisher_id → publishers).',
          'Create `sales (sale_id INT PRIMARY KEY AUTO_INCREMENT, book_id INT NOT NULL, sale_date DATE NOT NULL, quantity INT NOT NULL CHECK (quantity > 0), unit_price DECIMAL(8,2) NOT NULL, FOREIGN KEY (book_id) REFERENCES books(book_id))` as the child of books.',
        ],
        type: 'challenge',
        primaryTable: 'books',
        ...
        hints: [{ level: 1, text: 'Order matters: publishers and authors before books, and books before sales, so every foreign key reference exists. If the tables already exist from earlier tasks, DROP TABLE IF EXISTS them first (children before parents).' }],
        validation: {           requiredColumns: ['publisher_id', 'name', 'author_id', 'book_id', 'title', 'genre', 'price', 'quantity_in_stock', 'sale_id', 'sale_date', 'quantity', 'unit_price'],           targetTable: 'books', expectedRowCount: 1,         },
      ...
      },
      {
        ...
        instructions: [
          ...
          "Execute an EXPLAIN query selecting from `books` where `title` equals 'Atomic Habits'.",
          'Confirm the optimizer uses `idx_books_title` with access type `ref`.',
        ],
        type: 'challenge',
        primaryTable: 'books',
