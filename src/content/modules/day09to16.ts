...
export const DAY_09_MODULE: ModuleData = {
  ...
  concepts: [
    ...
    // =========================================================================
    {
      id: 'aggregate-functions-overview',
      order: 1,
      title: '1. Aggregate Functions (COUNT, SUM, AVG, MIN, MAX)',
      shortDescription: 'Summarize thousands of rows into actionable summary metrics.',
      theory: {
        summary: 'Aggregate functions process multiple rows and collapse them into a single scalar value. SQL provides 5 core aggregate functions: COUNT, SUM, AVG, MIN, and MAX.',
        introTable: {
          tableName: 'products',
          description: 'Product catalog with price and stock levels',
        ...
        explanation: [
          '### 1. The Core 5 Aggregate Functions',
          '• `COUNT(*)`: Total count of rows in the table.',
          '• `COUNT(column)`: Counts non-NULL values in that column.',
          '• `SUM(column)`: Total sum of numeric values.',
          '• `AVG(column)`: Mathematical average (mean).',
          '• `MIN(column)` / `MAX(column)`: Lowest and highest values in that column.',
          'QUESTION_BLOCK::Single-Row Collapse::When using aggregates without GROUP BY, the entire result set is collapsed into exactly one single summary row.',
        ],
        stepBreakdowns: [
          {
            stepNumber: 1,
            stepTitle: 'Step 1: Computing Global Metrics',
            sqlSnippet: 'SELECT COUNT(*) AS total_items, AVG(price) AS avg_price, MIN(price) AS min_price, MAX(price) AS max_price\nFROM products;',
            explanation: 'Processes all product rows and returns one summary row.',
            tableData: {
              tableName: 'Summary Result',
              columns: ['total_items', 'avg_price', 'min_price', 'max_price'],
              highlightedColumns: ['total_items', 'avg_price'],
              rows: [
                [20, 114.62, 12.50, 420.00],
              ],
            },
          },
        ],
        syntaxBlocks: [
          {
            title: 'Aggregate query syntax',
            sql: 'SELECT COUNT(*) AS total_rows, AVG(price) AS avg_price, SUM(quantity_in_stock) AS total_stock\nFROM products;',
            description: 'Computes overall totals and averages.',
          },
        ],
        keyTakeaway: 'Aggregate functions summarize multiple records into a single value.',
        exampleQuery: 'SELECT COUNT(*) AS total_products, AVG(price) AS avg_price FROM products;',
        exampleQueryExplanation: 'Calculates total product count and average price across the entire catalog.',
        liveDemoSql: 'SELECT COUNT(*) AS total_products, AVG(price) AS avg_price, MIN(price) AS min_price, MAX(price) AS max_price FROM products;',
        liveDemoNotes: 'Displays global inventory statistics.',
        mcqs: [
          {
        ...
        masteryPoints: ['Use COUNT, SUM, AVG, MIN, MAX', 'Understand COUNT(*) vs COUNT(col) with NULLs'],
      },
      tasks: [
        {
          id: 'day09-c1-t1',
          title: 'Task 1: Catalog Price & Stock Aggregates',
          description: 'Calculate the total product count, average price, and total stock quantity across all products.',
          instructions: [
            'Select `COUNT(*) AS total_products`, `AVG(price) AS avg_price`, and `SUM(quantity_in_stock) AS total_stock` from `products`.',
            'End with a semicolon (;).',
          ],
          type: 'guided',
          primaryTable: 'products',
          initialSql: 'SELECT COUNT(*) AS total_products, AVG(price) AS avg_price, SUM(quantity_in_stock) AS total_stock\nFROM products;',
          solutionSql: 'SELECT COUNT(*) AS total_products, AVG(price) AS avg_price, SUM(quantity_in_stock) AS total_stock FROM products;',
          solutionExplanation: 'Computes global count, average, and sum for the products catalog.',
          hints: [{ level: 1, text: 'Use `COUNT(*)`, `AVG(price)`, and `SUM(quantity_in_stock)`.' }],
          validation: {
            targetTable: 'products',
            requiredColumns: ['total_products', 'avg_price', 'total_stock'],
            expectedRowCount: 1,
          },
          successMessage: 'Catalog statistics computed!',
        },
        {
          id: 'day09-c1-t2',
          title: 'Task 2: Customer Account Metrics',
          description: 'Count total customers and count how many customers have provided an email address.',
          instructions: [
            'Query the `customers` table.',
            'Select `COUNT(*) AS total_customers` and `COUNT(email) AS customers_with_email`.',
          ],
          type: 'independent',
          primaryTable: 'customers',
          initialSql: '-- Customer count aggregates\n',
          solutionSql: 'SELECT COUNT(*) AS total_customers, COUNT(email) AS customers_with_email FROM customers;',
          solutionExplanation: '`COUNT(*)` counts all customers, while `COUNT(email)` ignores NULL email values.',
          hints: [{ level: 1, text: 'Use `SELECT COUNT(*) AS total_customers, COUNT(email) AS customers_with_email FROM customers;`' }],
          validation: {
            targetTable: 'customers',
            requiredColumns: ['total_customers', 'customers_with_email'],
            expectedRowCount: 1,
          },
          successMessage: 'Perfect! You distinguished COUNT(*) from COUNT(col).',
        },
      ],
    },
    ...
    // =========================================================================
    {
      ...
      order: 2,
      title: '2. Grouping Rows with GROUP BY',
      ...
      tasks: [
        {
          ...
          type: 'guided',
          primaryTable: 'products',
          initialSql: 'SELECT category_id, COUNT(*) AS total_products\nFROM products\nGROUP BY category_id;',
          ...
          hints: [{ level: 1, text: 'Use `GROUP BY category_id;`' }],
          validation: {
            targetTable: 'products',
            requireGroupBy: true,
            expectedRowCount: 5,
        ...
        },
        {
          ...
          hints: [{ level: 1, text: 'Use `GROUP BY city;`' }],
          validation: {
            targetTable: 'customers',
          ...
          successMessage: 'Spot on! Customers counted by city.',
        },
      ],
    },
    ...
    // =========================================================================
    {
      id: 'filtering-with-having',
      order: 3,
      title: '3. Filtering Groups with HAVING (vs WHERE)',
      shortDescription: 'Post-aggregation group filters.',
      theory: {
        summary: 'To filter grouped aggregate results, use `HAVING`. `WHERE` filters individual rows before grouping, while `HAVING` filters aggregate group summaries after grouping.',
        introTable: {
          tableName: 'products',
          description: 'Category averages comparison',
          columns: ['category_id', 'COUNT(*)', 'AVG(price)'],
          rows: [
            [1, 4, 114.99],
            [2, 4, 39.37],
            [3, 4, 219.50],
        ...
        explanation: [
          ...
          '• `HAVING`: Filters group summaries **after** aggregation.',
          'QUESTION_BLOCK::Why WHERE cannot filter aggregates::Because WHERE evaluates before GROUP BY collapses rows into buckets!',
        ],
        stepBreakdowns: [
          {
            stepNumber: 1,
            stepTitle: 'Step 1: Filtering Groups with HAVING',
            sqlSnippet: 'SELECT category_id, COUNT(*) AS total_products, AVG(price) AS avg_price\nFROM products\nGROUP BY category_id\nHAVING AVG(price) > 100;',
            explanation: 'Computes category metrics and keeps only categories with average price above $100.',
            tableData: {
      ...
      tasks: [
        {
          ...
          type: 'guided',
          primaryTable: 'products',
          initialSql: 'SELECT category_id, AVG(price) AS avg_price\nFROM products\nGROUP BY category_id\nORDER BY avg_price DESC;',
  ...
  challenge: {
    ...
    tasks: [
      ...
      {
        id: 'day09-hw-3',
        title: 'Task 3: Categories with average price above $100 (HAVING)',
        description: 'Categories with average price above $100 (HAVING).',
        instructions: [
          'Select `category_id` and `AVG(price) AS avg_price` from `products` grouped by `category_id` having `AVG(price) > 100`.',
          'End with a semicolon (;).',
        ],
        type: 'challenge',
        primaryTable: 'products',
        initialSql: '-- Task 3: Categories with average price > 100 (HAVING)\n',
        solutionSql: 'SELECT category_id, AVG(price) AS avg_price FROM products GROUP BY category_id HAVING AVG(price) > 100;',
        solutionExplanation: 'Uses `HAVING AVG(price) > 100` to filter for premium categories.',
        hints: [{ level: 1, text: 'Use `GROUP BY category_id HAVING AVG(price) > 100;`' }],
        validation: {
          targetTable: 'products',
          requireGroupBy: true,
          requireHaving: true,
        ...
        stepBreakdowns: [
          {
            ...
            explanation: 'Aggregates multiple metrics per category and keeps categories with average price > $50.',
            tableData: {
      ...
      tasks: [
        {
          ...
          type: 'guided',
          primaryTable: 'products',
          initialSql: 'SELECT category_id, COUNT(*) AS product_count, AVG(price) AS avg_price\nFROM products\nGROUP BY category_id\nHAVING AVG(price) > 15\nORDER BY product_count DESC;',
...
export const DAY_11_MODULE: ModuleData = {
  ...
  type: 'module',
  milestoneId: 'milestone-2',
  description: 'Understand relational keys (PK & FK), connect tables using INNER JOIN and LEFT JOIN, and use table aliases cleanly.',
  estimatedMinutes: 90,
  completionLearnings: [
    ...
    'Preserve unmatched left-table records using LEFT JOIN (such as customers with 0 orders)',
  ...
  concepts: [
    ...
    // =========================================================================
    {
      ...
      theory: {
        summary: 'A primary key (PK) uniquely identifies a row in its own table (e.g. `customer_id` in `customers`). A foreign key (FK) is a column in another table storing that same value to point back to it (e.g. `orders.customer_id`).',
        introTable: {
          ...
          rows: [
            [1, 'Wireless Mouse', 2, 2, 'Accessories'],
            [2, 'Mechanical Keyboard', 1, 1, 'Electronics'],
            [3, 'USB-C Cable (2m)', 2, 2, 'Accessories'],
            [4, 'Ergonomic Desk Chair', 3, 3, 'Office Furniture'],
            [5, 'Noise-Cancelling Headphones', 5, 5, 'Audio & Video'],
          ],
        },
        explanation: [
        ...
        stepBreakdowns: [
          {
            ...
            tableData: {
              ...
              rows: [
                ...
                ['Ergonomic Desk Chair', 'Office Furniture'],
                ['Noise-Cancelling Headphones', 'Audio & Video'],
              ],
            },
          },
        ],
      ...
      tasks: [
        {
          ...
          initialSql: 'SELECT p.name AS product_name, c.name AS category_name\nFROM products p\nINNER JOIN categories c ON p.category_id = c.category_id;',
    ...
    // =========================================================================
    {
      id: 'left-join-unmatched',
      order: 2,
      title: '2. LEFT JOIN & Preserving Unmatched Records',
      shortDescription: 'Keep all left-table rows even when no matching right-table record exists.',
      theory: {
        summary: 'A `LEFT JOIN` returns ALL rows from the left table. If there is no match in the right table, columns from the right table are filled with `NULL`. This is essential for questions like "Show every customer and their order count, including customers with 0 orders."',
        introTable: {
          ...
          rows: [
            [1, 'Rahim Chowdhury', 101],
            [1, 'Rahim Chowdhury', 104],
            [5, 'Fatima Noor', null],
            [9, 'Sara Khan', null],
          ],
        },
        explanation: [
          ...
          '`FROM customers c LEFT JOIN orders o ON c.customer_id = o.customer_id` keeps every customer.',
          'For customers with no orders, `o.order_id` is `NULL`.',
          'QUESTION_BLOCK::Counting with LEFT JOIN::Use `COUNT(o.order_id)` instead of `COUNT(*)`. `COUNT(*)` counts the row itself (giving 1 for 0 orders), whereas `COUNT(o.order_id)` correctly yields 0 when order_id is NULL!',
        ],
        stepBreakdowns: [
          {
            stepNumber: 1,
            stepTitle: 'Step 1: Preserving Customers with Zero Orders',
            sqlSnippet: 'SELECT c.customer_id, c.name, COUNT(o.order_id) AS order_count\nFROM customers c\nLEFT JOIN orders o ON c.customer_id = o.customer_id\nGROUP BY c.customer_id, c.name;',
            explanation: 'Counts orders per customer while preserving customers who have placed zero orders.',
            tableData: {
              tableName: 'Customer Order Roster',
        ...
        syntaxBlocks: [
          {
            ...
            description: 'Preserves all customers and computes accurate order count.',
          },
        ],
        keyTakeaway: 'Use LEFT JOIN when you want all left-table rows preserved regardless of right-table matches.',
        exampleQuery: 'SELECT c.customer_id, c.name, COUNT(o.order_id) AS order_count FROM customers c LEFT JOIN orders o ON c.customer_id = o.customer_id GROUP BY c.customer_id, c.name;',
        exampleQueryExplanation: 'Counts orders for every customer including zero-order accounts.',
        liveDemoSql: 'SELECT c.customer_id, c.name, COUNT(o.order_id) AS order_count FROM customers c LEFT JOIN orders o ON c.customer_id = o.customer_id GROUP BY c.customer_id, c.name LIMIT 5;',
        liveDemoNotes: 'Displays customer order counts with 0s preserved.',
        mcqs: [
          {
            question: 'Why should you use `COUNT(o.order_id)` instead of `COUNT(*)` when counting orders with a LEFT JOIN?',
            options: [
              'A. Because COUNT(*) is deprecated in SQL',
              'B. Because COUNT(*) counts the row itself (producing 1 even for customers with zero orders), whereas COUNT(o.order_id) ignores NULLs and correctly returns 0',
              'C. Because COUNT(o.order_id) executes faster',
              'D. Because orders must have a name',
            ],
            correctIndex: 1,
            explanation: '`COUNT(column)` ignores NULL values, whereas `COUNT(*)` counts all rows in the joined result set.',
          },
        ],
        masteryPoints: ['Use LEFT JOIN to include unmatched records', 'Count related keys accurately using COUNT(fk)'],
      },
      tasks: [
        {
          id: 'day11-c2-t1',
          title: 'Task 1: Every Customer with Order Count',
          description: 'List every customer with their total order count, including customers with zero orders.',
          instructions: [
            'Select `c.customer_id`, `c.name`, `COUNT(o.order_id) AS order_count` from `customers c` LEFT JOIN `orders o` ON `c.customer_id = o.customer_id`.',
            'Group by `c.customer_id`, `c.name`.',
            'End with a semicolon (;).',
          ],
          type: 'guided',
          ...
          initialSql: 'SELECT c.customer_id, c.name, COUNT(o.order_id) AS order_count\nFROM customers c\nLEFT JOIN orders o ON c.customer_id = o.customer_id\nGROUP BY c.customer_id, c.name;',
          ...
          solutionExplanation: 'Uses LEFT JOIN and COUNT(o.order_id) to accurately count orders per customer.',
          ...
          successMessage: 'All customer order counts computed including 0s!',
        },
        {
          id: 'day11-c2-t2',
          title: 'Task 2: Suppliers and Supplied Product Count',
          description: 'List all suppliers and the count of products they supply, including suppliers with 0 products.',
          instructions: [
            'Query `suppliers s` LEFT JOIN `products p` ON `s.supplier_id = p.supplier_id`.',
            'Select `s.supplier_id`, `s.name`, and `COUNT(p.product_id) AS product_count`.',
            'Group by `s.supplier_id`, `s.name`.',
          ],
          type: 'independent',
          primaryTable: 'suppliers',
          secondaryTables: ['products'],
          initialSql: '-- Supplier product count (LEFT JOIN)\n',
          solutionSql: 'SELECT s.supplier_id, s.name, COUNT(p.product_id) AS product_count FROM suppliers s LEFT JOIN products p ON s.supplier_id = p.supplier_id GROUP BY s.supplier_id, s.name;',
          solutionExplanation: 'Preserves all 10 suppliers while counting products per supplier.',
          hints: [{ level: 1, text: 'Use `LEFT JOIN products p ON s.supplier_id = p.supplier_id GROUP BY s.supplier_id, s.name;`' }],
          validation: {
      ...
      tasks: [
        {
          ...
          initialSql: 'SELECT c.name, COUNT(DISTINCT o.order_id) AS order_count, SUM(oi.quantity * oi.unit_price) AS total_spent\nFROM customers c\nJOIN orders o ON c.customer_id = o.customer_id\nJOIN order_items oi ON o.order_id = oi.order_id\nGROUP BY c.customer_id, c.name;',
      ...
      tasks: [
        {
          ...
          primaryTable: 'customers',
          secondaryTables: ['orders'],
          initialSql: 'SELECT c.name, COUNT(o.order_id) AS valid_orders\nFROM customers c\nJOIN orders o ON c.customer_id = o.customer_id\nWHERE o.status != \'cancelled\'\nGROUP BY c.customer_id, c.name\nHAVING COUNT(o.order_id) >= 1\nORDER BY valid_orders DESC\nLIMIT 5;',
        ...
        },
        {
          ...
          primaryTable: 'categories',
          secondaryTables: ['products'],
          initialSql: '-- 7-step pipeline on categories and products\n',
    ...
    tasks: [
      {
        ...
        primaryTable: 'customers',
        secondaryTables: ['orders'],
        initialSql: '-- Task 1: Full 7-clause pipeline query\n',
      ...
      tasks: [
        {
          ...
          initialSql: 'SELECT p.name, SUM(oi.quantity) AS total_units_sold\nFROM products p\nJOIN order_items oi ON p.product_id = oi.product_id\nGROUP BY p.product_id, p.name\nORDER BY total_units_sold DESC;',
      ...
      tasks: [
        {
          ...
          primaryTable: 'customers',
          secondaryTables: ['orders', 'order_items'],
          initialSql: 'SELECT c.name, SUM(oi.quantity * oi.unit_price) AS recent_spend\nFROM customers c\nJOIN orders o ON c.customer_id = o.customer_id\nJOIN order_items oi ON o.order_id = oi.order_id\nWHERE o.order_date >= CURDATE() - INTERVAL 60 DAY\nGROUP BY c.customer_id, c.name\nORDER BY recent_spend DESC;',
    ...
    tasks: [
      {
        ...
        primaryTable: 'customers',
        secondaryTables: ['orders', 'order_items'],
        initialSql: '-- Task 1: Polish multi-table spend report\n',
        solutionSql: 'SELECT c.name, SUM(oi.quantity * oi.unit_price) AS recent_spend FROM customers c JOIN orders o ON c.customer_id = o.customer_id JOIN order_items oi ON o.order_id = oi.order_id WHERE o.order_date >= CURDATE() - INTERVAL 60 DAY GROUP BY c.customer_id, c.name ORDER BY recent_spend DESC;',
        solutionExplanation: 'Combines multi-table joins, temporal filter, and group aggregation.',
        hints: [{ level: 1, text: 'Use `WHERE o.order_date >= CURDATE() - INTERVAL 60 DAY GROUP BY c.customer_id, c.name ORDER BY recent_spend DESC;`' }],
        validation: {
          targetTable: 'customers',
          requireJoin: true,
      ...
      tasks: [
        {
          ...
          initialSql: 'SELECT SUM(quantity * unit_price) AS total_revenue\nFROM order_items;',
