    ...
    tasks: [
      {
        ...
        instructions: [
          'Select `MONTH(order_date) AS mon` and `COUNT(*) AS order_count` from `orders`.',
          'GROUP BY `MONTH(order_date)`, ORDER BY `mon ASC`. End with a semicolon.',
        ],
        type: 'challenge',
        primaryTable: 'orders',
        initialSql: '-- Challenge: monthly order volume\n',
        solutionSql: 'SELECT MONTH(order_date) AS mon, COUNT(*) AS order_count FROM orders GROUP BY MONTH(order_date) ORDER BY mon ASC;',
        solutionExplanation: 'Extracts month numbers (2 through 8) from order_date and aggregates order counts per month across the single orders table.',
        hints: [
          { level: 1, text: 'Use MONTH(order_date) AS mon and COUNT(*) AS order_count, then GROUP BY MONTH(order_date) ORDER BY mon ASC;' },
        ],
        validation: {
          requireExactResult: true,
          targetTable: 'orders',
        ...
        successMessage: 'Monthly volume report delivered — date extraction with aggregation in one clean single-table query.',
      },
      {
        id: 'date-hw-2',
        ...
        instructions: [
          ...
          'ORDER BY `order_age_days DESC`. End with a semicolon.',
        ],
        type: 'challenge',
        primaryTable: 'orders',
        initialSql: '-- Challenge: open order aging audit\n',
        solutionSql: "SELECT order_id, customer_id, status, order_date, DATEDIFF(CURDATE(), order_date) AS order_age_days FROM orders WHERE status != 'delivered' ORDER BY order_age_days DESC;",
        solutionExplanation: 'Filters for non-delivered orders and measures day gaps using DATEDIFF(CURDATE(), order_date), ranking oldest open orders first.',
        hints: [
          { level: 1, text: 'Use WHERE status != \'delivered\' and compute DATEDIFF(CURDATE(), order_date) AS order_age_days.' },
        ],
        validation: {
          requireExactResult: true,
          targetTable: 'orders',
          requiredColumns: ['order_id', 'customer_id', 'status', 'order_date', 'order_age_days'],
          requireFunction: 'DATEDIFF',
          requireWhere: true,
          requireOrderBy: [{ column: 'order_age_days', direction: 'DESC' }],
          expectedRowCount: 7,
        },
        successMessage: 'Open order aging ladder built — relative date math and filtering working together!',
      },
      {
        id: 'date-hw-3',
