      ...
      tasks: [
        {
          ...
          instructions: [
            ...
            'Apply a 50% price reduction to all products in category 1 (`price = price * 0.5`).',
            'Roll back the transaction to discard the speculative change.',
            'Execute a verification query counting products where `price < 20` (aliased as `half_price`) to prove catalog integrity was preserved.',
          ],
          type: 'guided',
          primaryTable: 'products',
          secondaryTables: ['categories'],
        ...
        },
        {
          ...
          instructions: [
            ...
            'Update products in category 1 with prices of 20 or higher to half their current price.',
            'Commit the transaction to permanently persist the reviewed price changes.',
            'Verify the final state by selecting `product_id`, `name`, and `price` for products in category 1, ordered by `price` ascending.',
          ],
          type: 'independent',
          primaryTable: 'products',
          secondaryTables: ['categories'],
    ...
    tasks: [
      ...
      },
      {
        ...
        instructions: [
          ...
          "Update `orders` by setting `status` to 'shipped' specifically for orders with a 'pending' status created prior to '2026-08-15'.",
          'Commit the transaction to apply the updates safely.',
          "Run a verification query selecting `order_id` and `status` from `orders` for rows with status 'shipped', ordered by `order_id`.",
        ],
        type: 'challenge',
        primaryTable: 'orders',
