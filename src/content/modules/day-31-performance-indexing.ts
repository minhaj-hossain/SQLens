      ...
      tasks: [
        {
          ...
          instructions: [
            ...
            'Observe that without a filter or index, the planner specifies type `ALL` (full table scan) with a null key.',
            'Terminate your statement with a semicolon.',
          ],
          type: 'guided',
          primaryTable: 'products',
          initialSql: '-- See how the engine plans a whole-table read\n-- Type the EXPLAIN statement below:',
        ...
        },
        {
          ...
          instructions: [
            ...
            'Verify that despite having a WHERE clause, the planner still executes a full scan (`ALL`) because `price` is unindexed.',
            'Terminate your statement with a semicolon.',
          ],
          type: 'independent',
          primaryTable: 'products',
      ...
      tasks: [
        {
          ...
          instructions: [
            ...
            'Verify that the automatic primary key index achieves optimal plan metrics (`type: const`, `key: PRIMARY`, and exactly 1 row examined).',
            'Terminate your statement with a semicolon.',
          ],
          type: 'guided',
          primaryTable: 'products',
        ...
        },
        {
          ...
          type: 'independent',
          primaryTable: 'products',
          initialSql: '-- 1. Create the index on supplier_id\n-- 2. EXPLAIN the supplier lookup to see the plan change',
    ...
    },
    {
      ...
      theory: {
        ...
        explanation: [
          ...
          '1 `EXPLAIN SELECT * FROM products WHERE supplier_id = 2;` → type ALL → a scan.',
          '2 `CREATE INDEX idx_products_supplier ON products(supplier_id);`',
          '3 `EXPLAIN SELECT * FROM products WHERE supplier_id = 2;` → type ref, key idx_products_supplier, rows 7.',
      ...
      tasks: [
        {
          ...
          instructions: [
            ...
            'Execute an EXPLAIN plan for a query filtering `products` where `category_id` equals 1.',
            'Confirm that the planner utilizes `idx_products_category` with access type `ref`.',
          ],
          type: 'guided',
          primaryTable: 'products',
          initialSql: '-- Optimize the category filter: create the index, then EXPLAIN',
        ...
        },
        {
          ...
          instructions: [
            ...
            'Execute an EXPLAIN query filtering `products` where `price > 50`.',
            'Confirm that the ordered index converts the scan into access type `range`.',
          ],
          type: 'independent',
          primaryTable: 'products',
    ...
    tasks: [
      {
        ...
        instructions: [
          ...
          'Confirm the initial scan behavior (`type: ALL`, `key: null`) prior to index optimization.',
          'Terminate your statement with a semicolon.',
        ],
        type: 'challenge',
        primaryTable: 'orders',
      ...
      },
      {
        ...
        instructions: [
          ...
          'Execute an EXPLAIN statement on `orders` filtering where `customer_id` equals 3.',
          'Verify that the access type changes from `ALL` to `ref` using `idx_orders_customer`.',
        ],
        type: 'challenge',
        primaryTable: 'orders',
      ...
      },
      {
        ...
        instructions: [
          ...
          'Immediately run an EXPLAIN query filtering `orders` where `customer_id` equals 3.',
          'Verify that dropping the index reverts the planner access type back to a full scan (`ALL`).',
        ],
        type: 'challenge',
        primaryTable: 'orders',
      ...
      },
      {
        ...
        instructions: [
          ...
          'Verify that the primary key automatically delivers `type: const` with `key: PRIMARY` without requiring custom indexing.',
          'Terminate your statement with a semicolon.',
        ],
        type: 'challenge',
        primaryTable: 'orders',
