  ...
  concepts: [
    ...
    },
    {
      ...
      tasks: [
        {
          id: 'tx-c3-t1',
          title: 'Task 1 (Guided): Atomic Failure — Trigger a Constraint Violation',
          description: 'Deliberately trigger a foreign-key violation inside a transaction. This task expects an engine error — that error is the intended outcome and demonstrates how constraints guard atomicity.',
          instructions: [
            'Open a transaction.',
            'Attempt to insert a product referencing a non-existent category (category_id 999) into the `products` table. Use any valid values for the remaining required columns.',
            'Observe the foreign-key constraint error that fires. Do not fix the violation — the rejected insert is the expected result for this task.',
          ],
          type: 'guided',
          primaryTable: 'products',
        ...
        {
          id: 'tx-c3-t2',
          title: 'Task 2 (Independent): Roll Back the Failed Batch and Verify',
          description: 'With the transaction still open from the previous task, roll the entire batch back and confirm that no rows survived — not even the ones that appeared valid.',
          instructions: [
            'Roll back the open transaction.',
            'Write a SELECT that checks for the existence of any rows that were inserted during the transaction. Verify that zero rows survived.',
          ],
          type: 'independent',
          primaryTable: 'products',
