      ...
      tasks: [
        {
          ...
          instructions: [
            ...
            'Insert the flash-sale product into `products (name, supplier_id, category_id, price, quantity_in_stock, reorder_level)` using the values provided above.',
            'Commit the transaction to make the inserted row durable.',
          ],
          type: 'guided',
        ...
        },
        {
          ...
          instructions: [
            'Open a transaction.',
            'Insert a new order into `orders (customer_id, order_date, status)` for customer 2, dated 2026-08-26, with status pending.',
            'Insert the corresponding line item into `order_items (order_id, product_id, quantity, unit_price)` referencing order 19, product 1, quantity 2, at unit price 15.99.',
            'Commit the transaction so both rows become durable together.',
          ],
          type: 'challenge',
          primaryTable: 'orders',
      ...
      tasks: [
        {
          ...
          instructions: [
            'Open a transaction.',
            'Apply a 10% price increase to product 1 using an UPDATE statement.',
            'Query the price of product 1 to observe the provisional value inside the open transaction.',
            'Roll back the transaction and query the price again to confirm it has returned to its original value of 15.99.',
          ],
          type: 'guided',
          primaryTable: 'products',
        ...
        },
        {
          ...
          instructions: [
            ...
            'Confirm the price column shows 15.99 — the pre-transaction value restored by ROLLBACK.',
          ],
          type: 'independent',
          primaryTable: 'products',
      ...
      tasks: [
        {
          ...
          instructions: [
            ...
            'Use any valid values for the other required columns: `name`, `supplier_id`, `price`, `quantity_in_stock`, `reorder_level`.',
            'Run it as-is — no BEGIN, no ROLLBACK. The engine will reject it with a foreign-key constraint error. That error message is what this task is checking for.',
          ],
          type: 'guided',
          primaryTable: 'products',
          ...
          hints: [{ level: 1, text: 'Write just the INSERT with category_id = 999 — no BEGIN or ROLLBACK. The error the engine throws IS the passing result.' }],
          validation: {
            targetTable: 'products',
            expectFailure: true,
            expectedErrorCategory: 'FOREIGN_KEY',
          },
          successMessage: 'The constraint fired exactly as designed — now see how a transaction makes this atomic.',
          databaseLifecycle: 'fresh',
        },
        {
          id: 'tx-c3-t2',
          ...
          instructions: [
            ...
            'Insert a valid product (use any name, supplier_id 1, category_id 1, any price/stock/reorder values).',
            'Insert a second product that uses `category_id = 999` — this will trigger the foreign-key error.',
            'Issue `ROLLBACK;` to undo the entire batch.',
            'Finally, write a `SELECT name FROM products WHERE name IN (\'Limited Edition Mug\', \'Broken Item\');` to confirm that zero rows survived.',
          ],
          type: 'independent',
          primaryTable: 'products',
          initialSql: '-- BEGIN, two INSERTs (one bad), ROLLBACK, then verify\n',
          ...
          hints: [
            { level: 1, text: 'Start with BEGIN; then add both INSERTs. After the error from the second INSERT, ROLLBACK; and finish with the SELECT to prove zero rows landed.' },
          ],
          validation: {
            requireExactResult: true,
            targetTable: 'products',
    ...
    tasks: [
      ...
      },
      {
        ...
        instructions: [
          ...
          'Confirm that exactly 3 rows are returned — proof of successful, durable commitment.',
        ],
        type: 'challenge',
        primaryTable: 'products',
        setupSql:
